...
export default  async function ItemPage({
    ...
    if (!item){
        ...
        <div className="space-y-8 flex flex-col items-center mt-12">
          ...
          <p className="text-center">
            The item you&apos;re trying to view is invalid.<br/>Please go back and search for a different auction item.
          </p>
          <Button asChild>
    ...
    //const bids=[]; //if no one has bid the comment the previous to check how it will be shown
    ...
    return(
        <main className="container mx-auto py-12 space-y-8">
           <div className="flex gap-8">
            ...
            <h1 className={pageTitleStyles}><span className="font-normal">Auction for</span> {item.name}
            </h1>
            <Image 
            ...
            <div className="text-xl space-y-4">
                <div>
                Current Bid{" "}<span className="font-bold">₹{formatToDollar(item.currentBid)}</span>
                </div>
                <div>
                Starting Price of{" "}<span className="font-bold">₹{formatToDollar(item.startingPrice)}</span>
                </div>
                <div>Bid Interval <span className="font-bold">₹{formatToDollar(item.bidInterval)}</span>
                </div>
                </div>
            </div>
